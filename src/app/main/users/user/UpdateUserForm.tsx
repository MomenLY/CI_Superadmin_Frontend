...
const schema = z?.object({
  ...
  email: z.string().email("youMustEnterAValidEmail").optional(),
...
function UpdateUserForm() {
  ...
  const GetAllAdmins = useCallback(
    debounce(async () => {
      ...
      const modifiedData =
        roles &&
        roles
          .filter((item: any) => item.roleType !== 'enduser' && item.name !== 'SuperAdmin') // Exclude items with roleType 'enduser' or name 'SuperAdmin'
          .map((item: any) => ({
  ...
  const handleUpdate = useDebounce((res: any) => {
    if (res?.data?.updateCount === 0) {
      ...
      dispatch(
        showMessage({ message: t("userUpdatedSuccess"), variant: "success" })
      );
    }
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      if (errorMesssage) {
        dispatch(
          showMessage({
            message: errorMesssage || "User already exists",
            variant: "error",
          })
        );
        setIsLoading((prev) => !prev);
  ...
  return (
    ...
    >
      <OnionSubHeader title={t("editInfo")} subTitle={t("editInfoHelper")} />
      <form
        name="UserUpdateForm"
        noValidate
        spellCheck={false}
