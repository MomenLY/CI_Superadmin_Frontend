...
function RoleTable({ setKeyword, keyword, rules }: Props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      Cell: ({ row }) => <Typography>{row.original.roleType}</Typography>,
    },
    {
      ...
      Cell: ({ row }) =>
        ...
        row.original.users.length !== undefined && (
          <Stack direction="row">
            <AvatarGroup
              renderSurplus={(surplus) => (
                <span className="custom-surplus">+{surplus.toString()[0]}</span>
              )}
              max={4}
              spacing={11}
              sx={{
  ...
  const getRoleData = useCallback(
    debounce(async ({ pagination, keyword, sorting }) => {
      setIsLoading(true);
      try {
        const response = await getUserDetailsAPIWithSearch({
          pagination,
          keyword,
          sorting,
        });
        const filteredRoles = response.items && response.items.filter((role:any) => role.roleType !== 'enduser');
        setUsersWithRoles(filteredRoles);
          ...
          renderRowActions={({ row }) => (
            <Box sx={{ display: "flex", gap: "5px", alignItems: "center" }}>
              {rules?.editRole?.permission && (
                <IconButton
                  onClick={(event) =>
                    handleUpdate(row.original.roleId, row.original.roleType)
                  }
