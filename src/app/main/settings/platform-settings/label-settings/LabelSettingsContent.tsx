...
function LabelSettingsContent() {
  ...
  const onSubmit = async (formData) => {
    const modifiedFields = Object.keys(dirtyFields).filter(
    ...
    if (Object.keys(languageDefinitions).length > 0) {
      try {
        ...
        if (response) {
          dispatch(
            showMessage({ message: `${t("labelUpdated")}`, variant: "success" })
          );
          setIsLoading((prev) => !prev);
        }
      } catch (err) {
  ...
  const getDefaultValue = async () => {
    ...
    try {
      ...
      const languageItems = response.items.reduce((_languageItems, item) => {
        if (typeof _languageItems[item.LKey] === "undefined") {
          _languageItems[item.LKey] = {};
        }
        _languageItems[item.LKey]["default"] = item;
  ...
  return (
    <OnionPageOverlay>
      ...
      />
      <div className="mt-24">
        <Header label={""} content={t("labelSettingsHelperText")} />
            ...
            {Object.keys(languages).length > 0 &&
              Object.keys(languages).map((key) => (
                <Controller
                  ...
                  control={control}
                  defaultValue={
                    languages[key]?.custom?.LDefinition ||
                    languages[key]["default"]["LDefinition"]
                  }
                  render={({ field }) => {
                    const isError =
                      isDirty &&
                      (!allFields[key] ||
                        !(allFields[key] && allFields[key].trim()));
                    ...
                    if (search) {
                      const keywordToSplit = new Set(
                        defaultDefinition.match(new RegExp(search, "ig"))
                      );
                      const placeholders = {};
                      if (keywordToSplit.size > 0) {
                        for (const _keywordPiece of keywordToSplit) {
                          placeholders[_keywordPiece as string] = (
                            <Span>{_keywordPiece as any}</Span>
                          );
                        }
                        defaultDefinitionJSX = replacePlaceholdersWithJSX(
                          defaultDefinition,
                          placeholders
                        );
                      }
                    ...
                    return (
                      <FormControl className="flex flex-row items-center w-full">
                        <Typography className="w-1/3 semi-bold">
                          {defaultDefinitionJSX}
                        </Typography>
                        <TextField
                          {...field}
                          multiline={true}
